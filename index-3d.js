...
try {
    ...
    checkWallCollision() {
      ...
      if (collidingSegmentCenter) {
        ...
        if (distSqFromCenter > pow(TUNNEL_RADIUS - this.size.x * 0.5, 2)) {
          ...
          this.pos.x =
            collidingSegmentCenter.x + cos(angle) * constrainedRadius;
          this.pos.y =
            collidingSegmentCenter.y + sin(angle) * constrainedRadius;
        }
      }
    }
  ...
  class Projectile {
    ...
    draw() {
      ...
      if (this.type === "PLAYER_MISSILE") {
        ...
        let rotAngle = acos(
          constrain(
            createVector(0, 0, 1).dot(this.vel.copy().normalize()),
            -1,
            1
          )
        );
    ...
    checkCollision(target) {
      ...
      let collisionThresholdSq = pow(
        this.size.z * 0.5 + target.size.z * 0.5,
        2
      );
      return dSq < collisionThresholdSq;
    }
  ...
  class PowerUp {
    ...
    checkCollision(target) {
      ...
      let dSq =
        ...
        (this.pos.z - target.pos.z) ** 2;
      let collisionThresholdSq = pow(
        this.size.x * 0.7 + target.size.x * 0.5,
        2
      );
      return dSq < collisionThresholdSq;
    }
  ...
  function addTunnelSegment(zPos, preventSpawns = false) {
    ...
    if (!preventSpawns && gameState === "PLAYING") {
      ...
      if (random() < ENEMY_SPAWN_CHANCE) {
        ...
        let enemyZ =
          zPos +
          random(TUNNEL_SEGMENT_LENGTH * 0.2, TUNNEL_SEGMENT_LENGTH * 0.8);
  ...
  function drawStartScreen() {
    ...
    ambientLight(100);
    directionalLight(200, 200, 200, 0.5, 0.8, -1);
    pointLight(0, 0, 100, 0, 50, -100);
    if (player) player.draw(0, 50, 0);
  ...
  // --- Final Catch Block for Initialization Errors ---
} catch (error) {
  ...
  try {
    draw = function () {
      try {
        ...
        console.error("Error within error display draw():", drawError);
        document.body.innerHTML = `<pre style="color:red; font-family:monospace; font-size: 14px; padding: 10px; background: #222;">INIT ERROR (Display failed):\n${
    ...
    console.error("Error during error display setup:", e);
    if (!document.body.innerHTML.includes("INIT ERROR")) {
      document.body.innerHTML = `<pre style="color:red; font-family:monospace; font-size: 14px; padding: 10px; background: #222;">INIT ERROR (Display setup failed):\n${
        error.stack || error.message
      }\n\nPlease check the browser console.</pre>`;
    }
    if (typeof noLoop === "function") {
